...
class TestMdaNodeElem extends MdaNodeElemStatic {
...
class TestStreamElem extends TestMdaBaseElem {
  static const String STREAM_ID = 'TestStreamElem';
  int clickCount = 0;
  TestStreamElem(Element e) : super(e);

  Stream<int> getClickStream() => element.onClick.map((MouseEvent e) => ++clickCount);
}
...
void main() {
  ...
  test("MdaNodeElem two childs, childs MdaBaseElems, one with MdaStreamElem", () {
    ...
    MdaDomEntryHandle<TestStreamElem> testEntry = MdaDomEntryHandle<TestStreamElem>.entry(e, child2);
    expect(testEntry.node.getClickStream(), emitsInOrder([1, 2]));
